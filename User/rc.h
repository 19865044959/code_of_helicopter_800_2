...
struct PCM_XZ{
	bool PCM_Safe;//1-safe  0--error
	volatile bool last_PCM_Safe;
	float Right_Rate;//the right rate of PCM
	bool Take_Off;//1--takeoff  0--land
	int restart_count;//record the time of PCM_Fail
  bool RTL_Start_Flag; // 1--start 2--stop
  bool RTL_Init_Flag; //
};
extern struct PCM_XZ pcm_xz;
#define RC_NUM 1
