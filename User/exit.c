...
void EXTI15_10_IRQHandler(void)
{
	...
	if(__HAL_GPIO_EXTI_GET_IT(PWM_IN3_Pin) != RESET)
	{
		...
		if(HAL_GPIO_ReadPin(PWM_IN3_GPIO_Port,PWM_IN3_Pin)==GPIO_PIN_RESET)
		{
			...
			{
				if(ele->TimP[2].OUT > prePPM)
				{
					if((ele->TimP[2].OUT-prePPM)<1000)
					{
...
bool Exit_Calc(sEXIT *ele)
{
	...
	SlideFilt(&ele->DatFil[0],&ele->DatRel[0],1,&ele->Filt[0],1);
	...
	if(ele->RxFlag[2] == true)
	{
		...
		//EKF需要的更新时间
		ele->RxFlag[2] = false;
		ele->DatRel[2] = (ele->DatRaw[2] * 0.001f - 0.32f) * cos(ahrs[1].Ang[0]) * cos(ahrs[1].Ang[1]);//0.32f 是激光雷达零漂0.32米
		for(u8 i=1;i<EXIT_SLOPE_NUM;i++)
			ele->DatTmp[2][i-1]=ele->DatTmp[2][i];
		ele->DatTmp[2][EXIT_SLOPE_NUM-1]=ele->DatRel[2];
		float ab[2];
