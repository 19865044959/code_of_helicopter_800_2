...
#define MS5611_SPI1_CS_CLR	HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI1_NSS_Pin, GPIO_PIN_RESET)
#define MS5611_SPI1_CS_SET	HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI1_NSS_Pin, GPIO_PIN_SET)
#define MS5611_SPI4_CS_CLR	HAL_GPIO_WritePin(SPI4_NSS_GPIO_Port, SPI4_NSS_Pin, GPIO_PIN_RESET)
#define MS5611_SPI4_CS_SET	HAL_GPIO_WritePin(SPI4_NSS_GPIO_Port, SPI4_NSS_Pin, GPIO_PIN_SET)

void Ms5611_Spi_Tran(SPI_HandleTypeDef *hspi,u8 Cmd)
{
	if(hspi==&hspi1)MS5611_SPI1_CS_CLR;
	if(hspi==&hspi4)MS5611_SPI4_CS_CLR;
	HAL_SPI_Transmit(hspi,&Cmd,1,10);
	if(hspi==&hspi1)MS5611_SPI1_CS_SET;
	if(hspi==&hspi4)MS5611_SPI4_CS_SET;
}
...
bool Ms5611_Calc(sMS5611 *ele)
{
   ...
   SlideFilt(&ele->TmpFil,&ele->TmpRel,1,&ele->Filt,1);
	...
	ele->AltFil += filt_para * (ele->AltRel - ele->AltFil);
...
void Ms5611_Ctr(sMS5611 *ele)   //控制气压计温度
{
	...
	bell->htim->Instance->CCR2=(u16)Pid_tmp(pidTmp);   //CCR2 max为999     1ms
