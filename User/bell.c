...
void Bell_Init(sBELL *ele)
{
	...
	HAL_TIM_PWM_Start(ele->htim,TIM_CHANNEL_2);
	__HAL_ADC_ENABLE(ele->hadc);
	__HAL_TIM_SET_COMPARE(ele->htim,TIM_CHANNEL_2,50);
	ele->htim->Instance->ARR=1000;
	ele->htim->Instance->CNT=0;
	ele->hadc->Instance->CR2 |= ADC_CR2_DMA;
	ele->hadc->Instance->CR2 |= (uint32_t)ADC_CR2_SWSTART;  //软触发
	__HAL_DMA_DISABLE(ele->hadc->DMA_Handle);
	ele->hadc->DMA_Handle->Instance->PAR = (u32)&ele->hadc->Instance->DR;
	ele->hadc->DMA_Handle->Instance->NDTR = 2;
	ele->hadc->DMA_Handle->Instance->M0AR = (u32)ele->AdcRaw;
	DMA_CLEAR_FLAG_ALL(ele->hadc->DMA_Handle);
	__HAL_DMA_ENABLE(ele->hadc->DMA_Handle);
	HAL_Delay(10);
	Bell_Calc(ele);
...
void Bell_Loop_1ms(sBELL *ele)
{
	...
	if(--ele->TimeCnt==0)
	{
		if(ele->ToneIndex<ele->ToneNum)
		{
			...
			ele->htim->Instance->CCR1=(ele->TONE_ARRAY[ele->ToneIndex]>>1);
